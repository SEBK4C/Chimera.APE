...
int run_ingest(const Options& o) {
  ...
  for (DocWork& w : work) {
    ...
    if (w.modality != "text") {
      ...
      // Raw same-modality media vector — Gemma 4 12B's native end-state
      // embedding (LAST pooling over the projector+interleave forward pass).
      // Works on GPU with the patched llamafile (docs/GEMMA4-EMBEDDINGS.md).
